            ...
            if ($_SERVER['REQUEST_METHOD'] == "GET"){
                ...
                if(isset($_GET['search']) && isset($_GET['filter'])){
                    ...
                    if(!empty($_GET['search']) && !empty($_GET['filter'])){
                        ...
                        $filter = '%' . $_GET['filter'] . '%';
                        $sql = "SELECT title, content, image, post_id, u.username, profile_pic, pinned, upvotes, downvotes, p.category_id AS category_id, c.name AS category_name FROM post AS p JOIN category AS c ON p.category_id = c.id JOIN user AS u ON p.username = u.username WHERE (LOWER(title) LIKE :search OR LOWER(content)  LIKE :search) AND category_id = :filter ORDER BY p.created_at DESC, upvotes DESC";
                        $statement = $pdo->prepare($sql);
                        $statement->bindParam(':search', $search);
                        $statement->bindParam(':filter', $filter);
                    } elseif(!empty($_GET['search']) && empty($_GET['filter'])){
                        $search = '%' . strtolower($_GET['search']) . '%';
                        $sql = "SELECT title, content, image, post_id, u.username, profile_pic, pinned, upvotes, downvotes, p.category_id AS category_id, c.name AS category_name FROM post AS p JOIN category AS c ON p.category_id = c.id JOIN user AS u ON p.username = u.username WHERE LOWER(title) LIKE :search OR LOWER(content)  LIKE :search ORDER BY p.created_at DESC, upvotes DESC";
                        $statement = $pdo->prepare($sql);
                        $statement->bindParam(':search', $search);
                    }elseif(!empty($_GET['filter']) && empty($_GET['search'])){
                        $filter = $_GET['filter'];
                        $sql = "SELECT title, content, image, post_id, u.username, profile_pic, pinned, upvotes, downvotes, p.category_id AS category_id, c.name AS category_name FROM post AS p JOIN category AS c ON p.category_id = c.id JOIN user AS u ON p.username = u.username WHERE category_id LIKE :filter ORDER BY p.created_at DESC, upvotes DESC";
                        $statement = $pdo->prepare($sql);
                        $statement->bindParam(':filter', $filter);
                    }
                    else{
            $sql = "SELECT title, content, image, post_id, u.username, profile_pic, pinned, upvotes, downvotes, p.category_id AS category_id, c.name AS category_name FROM post AS p JOIN category AS c ON p.category_id = c.id JOIN user AS u ON p.username = u.username ORDER BY pinned DESC, p.created_at DESC, upvotes DESC";
                        $statement = $pdo->prepare($sql);
                    }
                }else{
            $sql = "SELECT title, content, image, post_id, u.username, profile_pic, pinned, upvotes, downvotes, p.category_id AS category_id, c.name AS category_name FROM post AS p JOIN category AS c ON p.category_id = c.id JOIN user AS u ON p.username = u.username ORDER BY pinned DESC, p.created_at DESC, upvotes DESC";
